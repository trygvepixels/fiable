...
import { FiSearch, FiGrid, FiList, FiArrowRight, FiCalendar, FiUser, FiClock } from "react-icons/fi";
import BlogCardSkeleton from "@/components/BlogCardSkeleton";
import Link from "next/link";

export default function BlogsListClient({ initialBlogs }) {
        ...
        <div className={`${
          ...
          {loading
            ...
            ? filteredPosts.map((post) => (
                <div
                  key={post.id}
                  onClick={() => router.push(`/blogs/${post.slug || post.id}`)}
    ...
    <section className="pt-32 pb-20 bg-[#F8F6F3]">
      <div className="max-w-7xl mx-auto px-6">
        <div className={`text-center transition-all duration-1000 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <h1 className="text-5xl lg:text-5xl font-light text-gray-900 mb-8 leading-tight">
             <span className=" ">Knowledge</span> {" "}
             <span className="text-[#234D7E] italic">Hub</span>
          </h1>
